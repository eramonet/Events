    ...
    <div class="d-block mb-4 mb-md-0">
        <nav aria-label="breadcrumb" class=" d-md-inline-block">
            <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                ...
                <li class="breadcrumb-item active" aria-current="page">Packages Options</li>
            </ol>
        </nav>
        <h2 class="h4">All Packages Options </h2>

    </div>
    <div class="btn-toolbar mb-2 mb-md-0">
        @if (Auth::guard('admin')->user()->hasPermission('packages-options-create'))
        <a href="{{ route('admin.packages-options.create') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
        ...
        <div class="btn-group ms-2 ms-lg-3">
            <a href="{{ route('admin.packages-options.export',) }}" class="btn btn-outline-success d-inline-flex align-items-center">
                {{-- <svg class="icon icon-xs me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg> --}}
        ...
        <tbody >
            ...
            <tr>
               ...
               <td>
                   ...
                   <p ><i class="fas fa-calendar-week text-info"></i> {{ \Carbon\Carbon::parse($option->created_at)->format('d/m/Y')}} </p>
                   <p  ><i class="fas fa-clock text-success"></i> {{ \Carbon\Carbon::parse($option->created_at)->format('h:i:s A')}} </p>
                ...
                <td>
                    ...
                    <div class="btn-group">
                        ...
                        <div class="dropdown-menu py-0">
                            <button data-bs-toggle="modal" data-bs-target="#modal-{{ $option->id }}" class="dropdown-item rounded-top" ><span class="fas fa-eye me-2"></span>View Details</button>

                            @if (Auth::guard('admin')->user()->hasPermission('packages-options-update'))
                            <a class="dropdown-item" href="{{ route('admin.packages-options.edit', $option->id) }}"><span class="fas fa-edit me-2"></span>Edit</a>
                            ...
                            @if (Auth::guard('admin')->user()->hasPermission('packages-options-update'))
                            <form  action="{{ route('admin.packages-options.restore', $option->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                            ...
                            <form class="delete-btn" action="{{ route('admin.packages-options.destroy', $option->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                ...
                <div class="modal fade" id="modal-{{ $option->id }}" tabindex="-1" role="dialog" aria-labelledby="modal-default" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="wrapper m-auto">
                                    ...
                                    <div class="d-flex mb-2">
                                        <img class="model-img" src="{{ $option->image_url }}" alt="{{ $option->title_en }} Image">
                                    </div>

                                    <ul class="list-group list-group-flush">
                                        <li class="list-group-item">Title In Arabic : {{ $option->title_ar }} </li>
                                        <li class="list-group-item">Title In English : {{ $option->title_en }} </li>
                                        ...
                                        <li class="list-group-item">Added By :
                                            ...
                                            <a class="btn btn-outline-primary" href="{{ route('admin.system-admins.index' ,[ 'admin_id'=>$option->admin->id ] )}}">{{ $option->admin->name }}</a>
                                            @else
                                               --
                                            @endif
                                        ...
                                        <li class="list-group-item">Category:
                                            ...
                                            <a class="btn btn-outline-primary" href="{{ route('admin.packages-options-categories.index' ,[ 'id'=>$option->category_id ] )}}">{{ $option->category->title_en ." - ". $option->category->title_ar}}</a>
                                        ...
                                        <li class="list-group-item">Added Date :  <i class="fas fa-calendar-week text-info"></i> {{ \Carbon\Carbon::parse($option->created_at)->format('d/m/Y')}}  </li>
                                        <li class="list-group-item">Added Time : <i class="fas fa-clock text-success"></i> {{ \Carbon\Carbon::parse($option->created_at)->format('h:i:s A')}}</li>
                            ...
                            <div class="modal-footer">
                                ...
                                @if (Auth::guard('admin')->user()->hasPermission('packages-options-update'))
                                <a class="btn btn-primary" href="{{ route('admin.packages-options.edit', $option->id) }}"><span class="fas fa-edit me-2"></span>Edit</a>
