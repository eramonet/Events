    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-2">
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
            ...
            <div class="btn-group ms-2 ms-lg-3">
                <a href="{{ route('admin.packages.export') }}"
            ...
            <tbody>
                ...
                @foreach ($packages as $package)
                    <tr>
                        ...
                        <td><img src="{{ $package->image_url }}" width="50px"></td>
                        ...
                        <td>
                            @if ($package->created_at)
                                <p><i class="fas fa-calendar-week text-info"></i>
                                    {{ \Carbon\Carbon::parse($package->created_at)->format('d/m/Y') }} </p>
                                <p><i class="fas fa-clock text-success"></i>
                                    {{ \Carbon\Carbon::parse($package->created_at)->format('h:i:s A') }} </p>
                            @else
                                ...
                            @endif
                        ...
                        <td>
                            <a href="{{ route('admin.packages.show' , $package->id) }}" class="btn btn-info"><span class="fas fa-eye"></span></a>

                            @if (Auth::guard('admin')->user()->hasPermission('packages-update'))
                                <a class="btn btn-primary" href="{{ route('admin.packages.edit', $package->id) }}"><span
                            ...
                            @if ($package->deleted_at)
                                @if (Auth::guard('admin')->user()->hasPermission('packages-update'))
                                    <form style="display: contents" action="{{ route('admin.packages.restore', $package->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                            ...
                            @else
                                @if (Auth::guard('admin')->user()->hasPermission('packages-delete'))
                                    <form style="display: contents" class="delete-btn" action="{{ route('admin.packages.destroy', $package->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="wrapper m-auto">
                                            ...
                                            <div class="d-flex mb-2">
                                                <img class="model-img" src="{{ $package->image_url }}"
                                                    alt="{{ $package->title_en }} Image">
                                            </div>

                                            <ul class="list-group list-group-flush">
                                                <li class="list-group-item">Title In Arabic : {{ $package->title_ar }}
                                                </li>
                                                <li class="list-group-item">Title In English : {{ $package->title_en }}
                                                </li>
                                                <li class="list-group-item">Price:
                                                    {{ number_format($package->real_price) }} AED </li>
                                                <li class="list-group-item">Price With Taxes:
                                                    {{ number_format($package->price_after_taxes) }} AED </li>
                                                ...
                                                <li class="list-group-item">Added By :
                                                    ...
                                                    @if ($package->admin)
                                                        <a class="btn btn-outline-primary"
                                                            href="{{ route('admin.system-admins.index', ['admin_id' => $package->admin->id]) }}">{{ $package->admin->name }}</a>
                                                    @else
                                                        --
                                                    @endif
                                                ...
                                                @if ($package->halls_count > 0)
                                                    <li class="list-group-item">Halls Count :
                                                        ...
                                                        <a class="btn btn-outline-primary"
                                                            href="{{ route('admin.halls.index', ['vendor_id' => $package->id]) }}">{{ $package->halls_count }}</a>
                                                ...
                                                @if ($package->vendor)
                                                    <li class="list-group-item">Vendor :
                                                        ...
                                                        <a class="btn btn-outline-primary"
                                                            href="{{ route('admin.vendors.index', ['vendor_id' => $package->vendor_id]) }}">{{ $package->vendor->title_en . ' - ' . $package->vendor->title_ar }}</a>
                                                ...
                                                @if ($package->hall)
                                                    <li class="list-group-item">Hall :
                                                        ...
                                                        <a class="btn btn-outline-primary"
                                                            href="{{ route('admin.halls.index', ['hall_id' => $package->hall_id]) }}">{{ $package->hall->title_en . ' - ' . $package->hall->title_ar }}</a>
                                                ...
                                                @if ($package->category)
                                                    <li class="list-group-item">Category :
                                                        ...
                                                        <a class="btn btn-outline-primary"
                                                            href="{{ route('admin.halls-categories.index', ['category_id' => $package->category_id]) }}">{{ $package->category->title_en . ' - ' . $package->category->title_ar }}</a>
                                                ...
                                                <li class="list-group-item">Added Time : <i
                                                    ...
                                                    {{ \Carbon\Carbon::parse($package->created_at)->format('h:i:s A') }}
                                                </li>
