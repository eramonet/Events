...
@section('content')
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-2">
        <div class="d-block mb-4 mb-md-0">
            ...
            <h2 class="h4">All Promo Codes</h2>
            ...
            <div class="accordion-item">
                ...
                <div id="collapseTwo" class="accordion-collapse collapse" aria-labelledby="headingTwo"
                    ...
                    <div class="accordion-body card card-body ">
                        ...
                        <form action="{{ route('admin.promo-codes.index') }}">
    ...
    <div class="card card-body border-0 shadow table-wrapper table-responsive">
            ...
            <tbody>
                ...
                @foreach ($promo_codes as $promo)
                    <tr>
                        ...
                        <td>
                            @if ($promo->created_at)
                                <p><i class="fas fa-calendar-week text-info"></i>
                                    {{ \Carbon\Carbon::parse($promo->created_at)->format('d/m/Y') }} </p>
                                <p><i class="fas fa-clock text-success"></i>
                                    {{ \Carbon\Carbon::parse($promo->created_at)->format('h:i:s A') }} </p>
                            @else
                                ...
                            @endif
                        ...
                        <div class="modal fade" id="modal-{{ $promo->id }}" tabindex="-1" role="dialog"
                            aria-labelledby="modal-default" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h2 class="h6 modal-title">{{ $promo->title }}</h2>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="wrapper m-auto">
                                            ...
                                            <ul class="list-group list-group-flush">
                                                <li class="list-group-item">Title : {{ $promo->title }} </li>

                                                @php

                                                    $exploded = explode('_', $promo->dedicated_to);
                                                    $imploded = implode(' ', $exploded);
                                                    $capital_letters = ucfirst($imploded);

                                                @endphp
                                                <li class="list-group-item">Dedicated To : {{ $capital_letters }} </li>

                                                @if ($promo->user)
                                                    <li class="list-group-item">For User :
                                                        <a class="btn btn-outline-primary"
                                                            href="{{ route('admin.users.index', ['user_id' => $promo->user->id]) }}">{{ $promo->user->name }}</a>

                                                    </li>
                                                @endif
                                                <li class="list-group-item">Type : {{ ucfirst($promo->type) }} </li>

                                                <li class="list-group-item">Value : {{ $promo->value }}
                                                    {{ $promo->type == 'percent' ? '%' : '' }} </li>


                                                <li class="list-group-item">Maximum Times of Use : <span
                                                        class="badge bg-success">
                                                        {{ $promo->maximum_times_of_use }}</span></li>
                                                <li class="list-group-item">Count of Use : <span class="badge bg-danger">
                                                        {{ $promo->count_of_use }}</span></li>
                                                ...
                                                <li class="list-group-item">Added By :
                                                    ...
                                                    @if ($promo->admin)
                                                        <a class="btn btn-outline-primary"
                                                            href="{{ route('admin.system-admins.index', ['admin_id' => $promo->admin->id]) }}">{{ $promo->admin->name }}</a>
                                                    @else
                                                        --
                                                    @endif
                                                ...
                                                <li class="list-group-item">Expired Date : <i
                                                        class="fas fa-calendar-week text-danger"></i>
                                                    {{ \Carbon\Carbon::parse($promo->expiration_date)->format('d/m/Y') }}
                                                </li>

                                                <li class="list-group-item">Added Date : <i
                                                        class="fas fa-calendar-week text-info"></i>
                                                    {{ \Carbon\Carbon::parse($promo->created_at)->format('d/m/Y') }} </li>
                                                <li class="list-group-item">Added Time : <i
                                                        class="fas fa-clock text-success"></i>
                                                    {{ \Carbon\Carbon::parse($promo->created_at)->format('h:i:s A') }}</li>
        ...
        <div class="card-footer px-3 border-0 d-flex flex-column flex-lg-row align-items-center justify-content-between">
            {!! $promo_codes->appends(request()->query())->links('pagination::bootstrap-5') !!}
