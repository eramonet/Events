                    ...
                    <tr>
                        ...
                        <td>
                            @if ($user->created_at)
                                ...
                                <p><i class="fas fa-clock text-success"></i>
                                    {{ \Carbon\Carbon::parse($user->created_at)->format('h:i:s A') }} </p>
                            @else
                                ...
                            @endif
                        ...
                        <td>
                            ...
                            <a href="{{ route('admin.users.show' , $user->id) }}"
                                class="btn btn-primary"><span class="fas fa-eye"></span></a>

                            @if (Auth::guard('admin')->user()->hasPermission('users-update'))
                                <a class="btn btn-info" href="{{ route('admin.users.edit', $user->id) }}"><span
                            ...
                            @if ($user->deleted_at)
                                @if (Auth::guard('admin')->user()->hasPermission('users-update'))
                                    <form style="display: contents" action="{{ route('admin.users.restore', $user->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                            ...
                            @else
                                @if (Auth::guard('admin')->user()->hasPermission('users-delete'))
                                    <form style="display: contents" class="delete-btn" action="{{ route('admin.users.destroy', $user->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
