    ...
    <div class="py-4">
        ...
        <div class="d-flex justify-content-between w-100 flex-wrap">
            <div class="mb-3 mb-lg-0">
                <h1 class="h4"> <i class="fa fa-house"></i> Vendors according to areas </h1>
                    ...
                    <div class="card border-0 shadow">
                        <div class="card-header">
                            <div class="row align-items-center">
                                <div class="col">
                                    <h2 class="fs-5 fw-bold mb-0">Latest 20 vendor Related to city</h2>
                                </div>
                                <div class="col text-end">
                                    <a href="{{ route('admin.vendor-admins.index') }}" class="btn btn-sm btn-primary">See
                                        All vendors</a>
                            ...
                            <table class="table align-items-center table-flush">
                                ...
                                <tbody >
                                    ...
                                    <tr>
                                       ...
                                       <td>
                                           @if ($admin->created_at)
                                           <p ><i class="fas fa-calendar-week text-info"></i> {{ \Carbon\Carbon::parse($admin->created_at)->format('d/m/Y')}} </p>
                                           <p  ><i class="fas fa-clock text-success"></i> {{ \Carbon\Carbon::parse($admin->created_at)->format('h:i:s A')}} </p>
                                        ...
                                        <td>
                                            ...
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu py-0">
                                                    <button data-bs-toggle="modal" data-bs-target="#modal-{{ $admin->id }}" class="dropdown-item rounded-top" ><span class="fas fa-eye me-2"></span>View Details</button>

                                                    @if (Auth::guard('admin')->user()->hasPermission('vendor-admins-update'))
                                                    <a class="dropdown-item" href="{{ route('admin.vendor-admins.edit', $admin->id) }}"><span class="fas fa-edit me-2"></span>Edit</a>
                                                    ...
                                                    @if ($admin->deleted_at)
                                                    @if (Auth::guard('admin')->user()->hasPermission('vendor-admins-update'))
                                                    <form  action="{{ route('admin.vendor-admins.restore', $admin->id) }}" method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                    ...
                                                    <form class="delete-btn" action="{{ route('admin.vendor-admins.destroy', $admin->id) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                        ...
                                        <div class="modal fade" id="modal-{{ $admin->id }}" tabindex="-1" role="dialog" aria-labelledby="modal-default" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h2 class="h6 modal-title">{{ $admin->admin->name }}</h2>
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="wrapper m-auto">
                                                            <div class="d-flex mb-2">
                                                                <img class="model-img" src="{{ $admin->image_url }}" alt="{{ $admin->admin->name }} Image">
                                                            </div>
                                                            <ul class="list-group list-group-flush">
                                                                <li class="list-group-item">Name : {{ $admin->admin->name }} </li>
                                                                <li class="list-group-item">Email : {{ $admin->email }} </li>
                                                                <li class="list-group-item">Phone :  {{ $admin->phone }}</li>
                                                                <li class="list-group-item">Category :  {{ $admin->admin->category ? $admin->admin->category->title_en :'-' }}</li>


                                                                <li class="list-group-item">Gender :  {{ $admin->admin->gender }}</li>

                                                                <li class="list-group-item">Role :  {{ $admin->admin->roles ? $admin->admin->roles[0]->name :'-' }} </li>
                                                                ...
                                                                <li class="list-group-item">Vendor  :
                                                                    ...
                                                                    <a class="btn btn-outline-primary" href="{{ route('admin.vendors.index' ,[ 'vendor_id'=>$admin->vendor->id ] )}}">{{ $admin->vendor->title_en .' - ' .$admin->vendor->title_ar}}</a>
                                                                    @else
                                                                       --
                                                                    @endif
                                                                ...
                                                                <li class="list-group-item">Added By :
                                                                    ...
                                                                    @if ($admin->added_by_admin)

                                                                    @if ($admin->added_by_admin && $admin->added_by_admin->roles && $admin->added_by_admin->roles[0]->name =='vendor-admin')

                                                                    <a class="btn btn-outline-primary" href="{{ route('admin.vendor-admins.index' ,[ 'admin_id'=>$admin->added_by_admin->id ] )}}">{{ $admin->added_by_admin->name }}</a>

                                                                    @else
                                                                    <a class="btn btn-outline-primary" href="{{ route('admin.system-admins.index' ,[ 'admin_id'=>$admin->added_by_admin->id ] )}}">{{ $admin->added_by_admin->name }}</a>
                                                                ...
                                                                <li class="list-group-item">Added Date :  <i class="fas fa-calendar-week text-info"></i> {{ \Carbon\Carbon::parse($admin->created_at)->format('d/m/Y')}}  </li>
                                                                <li class="list-group-item">Added Time : <i class="fas fa-clock text-success"></i> {{ \Carbon\Carbon::parse($admin->created_at)->format('h:i:s A')}}</li>
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        @if (Auth::guard('admin')->user()->hasPermission('vendor-admins-update'))
                                                        <a class="btn btn-primary" href="{{ route('admin.vendor-admins.edit', $admin->admin->id) }}"><span class="fas fa-edit me-2"></span>Edit</a>
