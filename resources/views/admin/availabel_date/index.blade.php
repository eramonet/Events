        ...
        <div class="d-block mb-4 mb-md-0">
            <nav aria-label="breadcrumb" class=" d-md-inline-block">
                <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">All Available DATES </li>
                </ol>
            </nav>
            <h2 class="h4">All Available DATES </h2>

        </div>
        <div class="btn-toolbar mb-2 mb-md-0">
            @if (Auth::guard('admin')->user()->hasPermission('packages-available-dates-and-times-create'))
                <a href="{{ route('admin.availabel_date.create') }}"
            ...
            <tbody>
                ...
                @foreach ($dates as $date)
                    <tr>
                        ...
                        <td>
                            @if ($date->created_at)
                                <p><i class="fas fa-calendar-week text-info"></i>
                                    {{ \Carbon\Carbon::parse($date->created_at)->format('d/m/Y') }} </p>
                                <p><i class="fas fa-clock text-success"></i>
                                    {{ \Carbon\Carbon::parse($date->created_at)->format('h:i:s A') }} </p>
                            @else
                                ...
                            @endif
                        ...
                        <td>
                            <button data-bs-toggle="modal" data-bs-target="#modal-{{ $date->id }}"
                            ...
                            @if (Auth::guard('admin')->user()->hasPermission('packages-available-dates-and-times-update'))
                                <a class="btn btn-primary" href="{{ route('admin.availabel_date.edit', $date->id) }}"><span
                            ...
                            @if ($date->deleted_at)
                                @if (Auth::guard('admin')->user()->hasPermission('packages-available-dates-and-times-update'))
                                    <form style="display: contents" action="{{ route('admin.availabel_date.restore', $date->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" class="btn btn-success">
                                            <span class="fa-solid fa-trash-can-arrow-up"></span>Restore
                                        </button>
                                    </form>
                                @endif
                            @else
                                @if (Auth::guard('admin')->user()->hasPermission('packages-available-dates-and-times-delete'))
                                    <form style="display: contents" class="delete-btn"
                                        action="{{ route('admin.availabel_date.destroy', $date->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="wrapper m-auto">
                                            ...
                                            <ul class="list-group list-group-flush">
                                                <li class="list-group-item">Hall Name [EN] : {{ $date->hall->title_en }}
                                                </li>

                                                <li class="list-group-item">Hall Name [AR] : {{ $date->hall->title_ar }}
                                                </li>

                                                <li class="list-group-item">Available Date : {{ $date->available_date }}
                                                </li>
                                                <li class="list-group-item">Time From : {{ $date->time_from }}
                                                </li>

                                                <li class="list-group-item">Time To : {{ $date->time_to }}
                                                </li>
                                                ...
                                                <li class="list-group-item">Added Date : <i
                                                    ...
                                                    {{ \Carbon\Carbon::parse($date->created_at)->format('d/m/Y') }} </li>
                                                <li class="list-group-item">Added Time : <i
                                                        class="fas fa-clock text-success"></i>
                                                    {{ \Carbon\Carbon::parse($date->created_at)->format('h:i:s A') }}</li>
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        @if (Auth::guard('admin')->user()->hasPermission('halls-categories-update'))
                                            <a class="btn btn-primary"
                                                href="{{ route('admin.availabel_date.edit', $date->id) }}"><span
