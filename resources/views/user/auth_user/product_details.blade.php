...
@section('title')
    {{ $getProduct->title_en }} | {{ $getProduct->category->title_en }} | {{ $getProduct->main_category->title_en }}
@endsection
@section('content')
                ...
                <div class="row">
                    <div class="col-lg-6" style="direction: ltr;">
                        <div class="product-slick">
                            <img src="{{ asset($getProduct->primary_image_url) }}" style="width: 30%; margin: auto; height: 30%;"
                                        class="img-fluid blur-up lazyload image_zoom_cls-0">
                            @foreach ($medias as $media)
                                <div><img src="{{ asset($media->image_url) }}" style="width: 30%; margin: auto; height: 30%;"
                    ...
                    <div class="col-lg-6 rtl-text">
                        <div class="product-right">
                            ...
                            <h3 class="price-detail">{{ $getProduct->real_price }} {{ __('trans.EGP') }}
                                @if ($getProduct->to > Carbon\Carbon::now()->toDateString())
                                    <del>{{ $getProduct->real_price < $getProduct->fake_price ? $getProduct->fake_price . trans('trans.EGP') : '' }}</del><span>{{ $getProduct->real_price < $getProduct->fake_price ? number_format(($getProduct->real_price / $getProduct->fake_price) * 100) . trans('trans.% off') : '' }}
                                @endif
                                </span>
                            </h3>
                            ...
                            <div class="product-buttons" id="add_to_cart_div">
                                ...
                                @if ($getProduct->sizes->count() > 0 || $getProduct->colors()->count() > 0)
                                    @if ($getProduct->stock >= 0)
                                        <a style="pointer-events: none;" id="cartEffect"
                                            ...
                                            class="btn btn-solid hover-solid btn-animation btn-danger">
                                            <i class="fa fa-shopping-cart me-1" aria-hidden="true"></i>

                                            {{ __('trans.add to cart') }}
                                    ...
                                    @else
                                        <p
                                            style="color:red; font-weight: bold;font-size:20px; width:fit-content; display:inline-block;
                                       ">
                                            {{ __('out of stock') }} </p>
                                    @endif
                                @else
                                ...
                                <a onclick="add_to_wishlist('{{ $getProduct->id }}')" class="btn btn-solid"><i
                                        ...
                                        aria-hidden="true"></i>{{ __('trans.wishlist') }}</a>
                            </div>
                            @if ($getProduct->shipping)
                                <div class="product-count">
                                    <ul>
                                        <li>
                            ...
                            <div class="border-product">
                                ...
                                <div class="product-icon">
                                    <ul class="product-social">
                                        <li><a
                                                href="https://www.facebook.com/sharer/sharer.php?u=https://newstore.eramoerp.com/product/{{ $getProduct->slug_en }}"><i
                                                    class="fa fa-facebook-f"></i></a></li>
                                        <li><a
                                                href="https://wa.me/?text=https://newstore.eramoerp.com/product/{{ $getProduct->slug_en }}"><i
                                                    class="fa fa-whatsapp"></i></a></li>
                                        <li><a
                                                href="https://twitter.com/intent/tweet?text=my share text&amp;url=https://newstore.eramoerp.com/product/{{ $getProduct->slug_en }}"><i
                                ...
                                <div class="border-product">
                                    ...
                                    <div class="bundle">
                                        <div class="bundle_img">
                                            <div class="img-box">
                                                <a
                                                    href="@if (session()->get('locale') == 'en') {{ url('product/' . $getProduct->slug_en) }}@else{{ url('product/' . $getProduct->slug_ar) }} @endif"><img
                                                        src="{{ asset('' . $getProduct->primary_image_url . '') }}"
                                            ...
                                            @foreach ($get2Products as $two)
                                                ...
                                                <div class="img-box">
                                                    <a
                                                        href="@if (session()->get('locale') == 'en') {{ url('product/' . $two->slug_en) }}@else{{ url('product/' . $two->slug_ar) }} @endif"><img
                                            ...
                                            <div class="theme_checkbox">
                                                @if (session()->get('locale') == 'en')
                                                    ...
                                                    <label>this product: @if (session()->get('locale') == 'en')
                                                        ...
                                                        @if ($getProduct->stock <= 0)
                                                            <p
                                                                style="color:red; font-weight: bold;font-size:16px; width:fit-content; display:inline-block;
                                                            ">
                                                                {{ __('out of stock') }} </p>
                                                        @endif
                                                    @else
                                                    ...
                                                    <label style="display: inline ;margin-right: 10px">this product:
                                                        @if (session()->get('locale') == 'en')
                                                            ...
                                                            ">
                                                                    {{ __('out of stock') }} </p>
                                                            @endif
                                                        @endif <span class="price_product"></span>
                                                ...
                                                @foreach ($get2Products as $two)
                                                    <label style="padding-left: 0px">
                                                        ...
                                                        @else
                                                            <span class="price_product">{{ $two->real_price . ' جنيه' }}
                                                                <del>{{ $two->fake_price <= $two->real_price ? '' : $two->fake_price . ' جنيه' }}</del></span>
                                                        @endif
                            ...
                            <div class="">
                                <div class="video-container">
                                    <iframe width="560" height="315"
                                        src="https://www.youtube.com/watch?v=lC2QcliExhY&ab_channel=YellowBrickCinema-RelaxingMusic"
                            ...
                            <div class="form-row row">
                                ...
                                <div class="col-md-12">
                                    <button class="btn btn-solid"
                                        type="submit">{{ __('trans.Submit YOur Review') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
                            ...
                            @foreach ($getproducts_with as $prod)
                                <div class="product-box product-style-4">
                                    <div class="img-wrapper">
                                        <div class="front">
                                            <a
                                                href="@if (session()->get('locale') == 'en') {{ url('product/' . $prod->slug_en) }}@else{{ url('product/' . $prod->slug_ar) }} @endif"><img
                                    ...
                                    <div class="product-info"
                                        ...
                                        <a
                                            href="@if (session()->get('locale') == 'en') {{ route('product.details', $prod->slug_en) }}@else{{ route('product.details', $prod->slug_ar) }} @endif">
                                            <h6>
                                                @if (session()->get('locale') == 'en')
                                                    {{ $prod->title_en }}@else{{ $prod->title_ar }}
                                                @endif
                                            </h6>
                                        </a>
                                        ...
                                        <div class="addtocart_btn">
                                            @if ($prod->stock > 0)
                                                <button class="add-button add_cart" title="Add to cart"
                                                    @if (session()->get('locale') == 'ar') style="margin-right: 82%;" @endif
