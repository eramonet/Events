...
public function getAdmins(Request $request){
    ...
    $admins = $this->admin::whereRoleIs(['vendor-admin'])->with(['roles','category','vendor','added_by_admin'])
...
public function getAdminsAdmin(Request $request,$useradmin)
{
    ...
    $admins = $this->admin::whereRoleIs(['vendor-admin'])->with(['roles', 'category', 'vendor', 'added_by_admin'])
        ->where('vendor_id', $useradmin->vendor_id)
        ->where('vendor_id','!=',$useradmin->vendor_id)
    ->where(function ($query) use ($request) {
            return $query->when($request->name, function ($q) use ($request) {
                return $q->where('name', 'like', '%' . $request->name . '%');
...
public function store(Request $request  ){
    ...
    $admin->attachRole('vendor-admin');
    $admin->attachPermissions($request->permissions);

    if($request->hasFile('image')){
        $imageName = UploadHelper::upload('admins_images', $request->file('image'), 180, 180);
...
public function getAdminById(int $id){
   return  $this->admin::whereRoleIs(['vendor-admin'])->withTrashed()->find($id);
