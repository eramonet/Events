...
class OrderService
{
    ...
    public function inprogressAction(Order $order)
    {
        ...
        $notification->update(['title_en'=>'order status','title_ar'=>'حالة الطلب','desc_en'=>'your order is inprogress now','desc_ar'=>'طلبك في الاعداد الان']);
        PushNotification::fire($user->fcm-token,"order status","your order is inprogress now", $order->id,"inprogress");
    ...
    public function deliveredAction(Order $order)
    {
        ...
        $user = User::where('id', $order->user_id)->first();
        $notification=Notification::where('order_id',$order->id)->first();
        $notification->update(['title_en'=>'order status','title_ar'=>'حالة الطلب','desc_en'=>'your order is delivered now','desc_ar'=>'طلبك في الطريق اليك']);
        PushNotification::fire($user->fcm-token, "order status", "your order is delivered now", $order->id, "delivered");
    ...
    public function cancelledAction(Order $order)
    {
        ...
        $user = User::where('id', $order->user_id)->first();
        $notification=Notification::where('order_id',$order->id)->first();
        $notification->update(['title_en'=>'order status','title_ar'=>'حالة الطلب','desc_en'=>'your order is cancelled now','desc_ar'=>'تم الغاء طلبك']);
        PushNotification::fire($user->fcm-token, "order status", "your order is cancelled", $order->id, "cancelled");
